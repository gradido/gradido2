// <out>/ir.json  ->  <out>/service_core/email_gen.h + <out>/email_gen.c
//
// The header sits under service_core/ so that the include spelling is the one
// fast-servers uses -- the two files are copied into service-core/ after the
...
const templates = ir.templates.map((t) => {
  ...
  const progs = []
  const max = { html: 0, subject: 0, refs: 0 }
...
${templates.map((t) => `    { "${t.name}", ${t.progBase}u, ${t.combos.length}u, ${t.slotOrder.length}u, GE_SLOTS_${UP(t.name)}, ${t.flags.length}u, ${t.flags.length ? `GE_FLAGS_${UP(t.name)}` : 'NULL'}, ${t.max.html}u, ${t.max.subject}u, ${t.max.refs}u }`).join(',\n')}
};
...
fs.mkdirSync(path.join(OUT_DIR, 'service_core'), { recursive: true })
fs.writeFileSync(path.join(OUT_DIR, 'service_core', 'email_gen.h'), h)
fs.writeFileSync(path.join(OUT_DIR, 'email_gen.c'), c)
...
console.error(`programs            ${PROGS.length} (${templates.length} templates x ${ir.locales.length} locales x variants x {html,subject})`)
console.error(`max static share    html ${Math.max(...templates.map((t) => t.max.html))} B, subject ${Math.max(...templates.map((t) => t.max.subject))} B, up to ${Math.max(...templates.map((t) => t.max.refs))} slot uses`)
