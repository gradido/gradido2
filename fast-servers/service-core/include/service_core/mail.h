 ...
 * has to do.
 */
#ifndef SERVICE_CORE_MAIL_H
#define SERVICE_CORE_MAIL_H
...
/** Bytes one rendered message may take when the caller names no size -- 32 KiB. */
#define SC_MAIL_MESSAGE_DEFAULT ((uint32_t)32 * 1024)
/** Round trip a single attempt is given before it is called failed. */
...
typedef struct sc_mail {
     ...
     * turned into the CRLF RFC 5322 wants.
     *
