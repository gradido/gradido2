...
## 3c. The e-mail renderer is copied in, not written here

Four files in `service-core/` have their source in `packages/email-native` and are outputs of
its build:

```text
include/service_core/email.h       the API, template-independent
src/email.c                        its runtime, template-independent
include/service_core/email_gen.h   generated from the pug templates
src/email_gen.c                    generated -- 420 KB, and not a review task
```

**Do not edit them in this tree.** Change the template or `email.c` in
...
implementations send different mails in the meantime.
...
it renders all 540 documents from the C and compares them against its checked-in snapshots,
