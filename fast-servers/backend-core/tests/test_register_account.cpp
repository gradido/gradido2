...
/** A migrated database with a home community, and a context an interaction can be given. */
class Registration : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        context_.db = db_;
        uv_mutex_init(&context_.db_lock);
    }

    void TearDown() override
    {
        uv_mutex_destroy(&context_.db_lock);
        sc_db_close(db_);
    ...
    sc_status register_one(const char *email)
    {
        ...
        const sc_status status =
            bc_register_account(&context_, "Einhorn", "Immond", email, "de", error, sizeof(error));
        last_error_ = error;
        return status;
    }
...
TEST_F(Registration, TellsTheAddressApartFromACollision)
{
    ...
    ASSERT_EQ(bc_user_create_account(db_, &first, &result, error, sizeof(error)), SC_OK) << error;
    ...
    ASSERT_EQ(bc_user_create_account(db_, &again, &result, error, sizeof(error)), SC_OK) << error;
...
TEST_F(Registration, SaysWhichGeneratedValueCollided)
{
    ...
    ASSERT_EQ(bc_user_create_account(db_, &same_code, &result, error, sizeof(error)), SC_OK)
        << error;
    ...
    EXPECT_NE(std::string(result.constraint).find("email_verification_code"), std::string::npos)
        << result.constraint;
    /* Rolled back whole: a collision leaves no member behind. */
    ...
    ASSERT_EQ(bc_user_create_account(db_, &same_id, &result, error, sizeof(error)), SC_OK) << error;
    EXPECT_EQ(result.outcome, BC_ACCOUNT_COLLIDED);
