...
A Gradido server hands out the frontend it serves. `../Architecture.md` has a deployment as one
binary next to a database — there is no nginx in front of it, and no second process whose only
job is files — so both implementations carry a static web server and both carry the same pages.

Neither implementation can *build* one. A page is vite's output, and this path has no JavaScript
in it at all. So the pages are built once, by the TypeScript toolchain, into `publish/` at the
repository root:

```text
publish/sites.json   which site is mounted where, and for every file its content type,
                     its ETag and whether the name carries a content hash
publish/frontend/    the files, exactly as they are handed out
publish/admin/       the next one, once packages/admin exists
```

`build.zig` reads that manifest at configure time and writes the files out as C, the same way it
writes `contracts/migrations` out as C — see `backend/include/backend/static_sites.h`. What is
embedded is what the manifest *names*, not what the directory happens to hold.

**The content type and the ETag come out of the manifest rather than out of this code**, and
that is the whole reason the manifest has them. Two static servers that inferred a type from an
extension separately would disagree about one of them eventually; two that hashed the bytes
separately would disagree about all of them. A deployment runs one implementation or the other,
and a client must not be able to tell which — the same rule `sc_http_reason` follows between the
two HTTP backends, one level up. It also means nothing is computed per request: a page is a
pointer, a length, and two header strings that were decided when the build ran.
...
change, which is what `service-core/src/email/templates.c` does for the mail templates — and
that one is 560 KB written by a codegen nobody reads. A frontend is fonts and photographs, it
changes with every CSS fix, and a repository whose diffs are dominated by regenerated byte
arrays is one where nobody reads a diff.

### Answering with bytes that are already there

A page is in the executable, so it is never copied to be sent: `sc_http_reply_static` hands the
socket the pointer. On h2o that spares the request pool a 134 KB copy of the font on every
visitor. On the fallback backend it is what makes a page *possible* — that server serialises a
response head into a 16 KiB buffer and copies an ordinary body into an 8 KiB one, and the font
is bigger than both. The head goes into the buffer and the body is written beside it, two
vectors in one `uv_write`, which is also one memcpy less for every other answer.

The rule the function carries is one line: **the bytes must outlive the process**. Anything on a
stack, in an arena or in a request pool is `sc_http_reply` and is copied.
