...
| `settings.json` | **28 runtime keys** — 21 instance settings from legacy's config, 4 that gradido2 invents for mirroring (`dht.mirror_*`), and in `user_settings.json` 3 member settings: 2 leaving `db/users.json` and `main_creation_group`, which replaces a table. Types, defaults and bounds are decided; the three publish-name enums are not |
...
| `test-vectors/` | **`jwt.json`: 37 vectors** for HS256 verification, run against `jwt.c` and against jose, with one declared divergence (a list-valued `aud`). The shape and the two runners are the pattern the subjects below are written to. **`master-seed.json`: 13 vectors** — which text is a master seed, the dht identity it derives, and the delegation the community key signs for it, every delegation signed by libp2p-ffi itself. **`public-url.json`: 41 vectors** — whether a URL names a host on the public internet, which decides a dht node's reachability. **`shard.json`: 7 vectors** — a community's shard and its shard and community topic keys, computed by a third implementation. Everything else is still missing |
